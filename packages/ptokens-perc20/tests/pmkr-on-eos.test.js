...
beforeEach(() => {
  pmkr = new pERC20({
    ...
    pToken: constants.pTokens.pMKR
  })
})

test('Should not issue less than 1000000000 pMKR', async () => {
  const amountToIssue = BigNumber('900000000')
  try {
    await pmkr.issue(amountToIssue, EOS_TESTING_ACCOUNT_NAME)
  } catch (_err) {
    expect(_err).to.be.equal('Impossible to issue less than 1000000000')
  }
})

test('Should issue 0.00002 pMKR using ETH', async () => {
  ...
  await pmkr.select()
  const { native_vault_address } = await pmkr.selectedNode.getInfo()
  ...
  await eth.sendSignedMethodTx(
    ...
    'approve',
    {
      ...
      gas: 200000,
      gasPrice: 75e9,
      contractAddress: constants.tokens.ethereum.mainnet.MKR,
      value: 0
    },
...
test('Should redeem 0.00002 pMKR on EOS', async () => {
